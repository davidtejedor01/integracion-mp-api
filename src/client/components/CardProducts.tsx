...
const CardProducts = ({ prod }: Props) => {
    const handleBuy = async () => {
        try {
            const res = await axios.post<preferenceRes>('http://localhost:3000/crearPreferencia', prod);
    ...
    return (
        ...
        transform hover:-translate-y-1 hover:scale-105 transition-all duration-300 mx-4 my-6 border border-blue-500">
            ...
            <button onClick={handleBuy} className="w-full bg-white text-blue-700 font-semibold py-2 rounded-lg shadow hover:bg-blue-100 transition-all">
